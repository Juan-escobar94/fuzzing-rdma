...
\begin{algorithm}[t]
  ...
  \LinesNumberedHidden
  \DontPrintSemicolon
  \KwIn{an unsigned integer $n$}
  \KwOut{a list of all core triads with at most $n$ vertices}
  \BlankLine
  \tcp{Finding a list of arms}
  $armlist \longleftarrow [\;]$\;
  \ForEach{arm $a$ with $\length(a) \leq n$}{
    \If{$\ACR_{a}(a)$ does not derive $id$}
    {Put $a$ in armlist\;}}
  \BlankLine
  \tcp{Assembling the arms to core triads}
  $triadlist \longleftarrow [\;]$\;
  \ForAll{$p_{1},p_{2}$ in armlist with combined at most $n$ vertices}{
    Let \(\theta = p_{1},p_{2}\) be a partial triad\;
      \If{$\ACR_{\theta}(\theta)$ does not derive $id$}
      {Drop $\theta$ and cache the two arms\;}}
  \ForAll{$p_{1},p_{2},p_{3}$ in armlist with combined at most $n$ vertices}{
    Let \(\mathbb{T} = p_{1},p_{2},p_{3}\) be a triad\;
      \If{$\mathbb{T}$ contains a pair of cached arms}
      {Drop $\mathbb{T}$ and continue\;}
      \If{$\AC_{\mathbb{T}}(\mathbb{T})$ derives $id$}
