...
void run_client(struct ibv_device *device) {
    ...
    struct conn_inf cinf;
    memset(&cinf, 0, sizeof cinf);

    char qpn[10];
    memset(qpn, 0, 10);
    sprintf(qpn, "%d", qp->qp_num);
    //sprintf(gid_send, "%s", gid.raw);
    xch_conn_inf_client("4791", qpn, gid.raw, &cinf);

    printf("successfully got infos from server\n");
    union ibv_gid their_gid;
    memset(&their_gid, 0, sizeof their_gid);

    memcpy(their_gid.raw, cinf.gid, sizeof their_gid.raw);


    ret = qp_init_to_rtr(qp, their_gid, cinf.qpn, destination_local_id, port_num);
    if (ret) {
